...
class MLPlay:
    ...
    def update(self, scene_info):
        ...
        while True:
            ...
            # 3.3 Put the code here to handle the scene information 放在此處以處理場景信息
            
            # 3.4 Send the instruction for this frame to the game process 發送此框架的指令到遊戲過程
            if not self.ball_served:
                ...
                if self.side == "1P":
                    ...
                    # record various intermediates (needed later for backprop) 記錄各種中間體（以後需要反向）
                    ...
                    if done: # an episode finished 一局結束
                        self.episode_number += 1
                        _score = [0,1]
                        # stack together all inputs, hidden states, action gradients, and rewards for this episode 將本集的所有輸入，隱藏狀態，動作梯度和獎勵堆疊在一起
                        epx = np.vstack(self.xs)
                        eph = np.vstack(self.hs)
                        epdlogp = np.vstack(self.dlogps)
                        epr = np.vstack(self.drs)
                        self.xs,self.hs,self.dlogps,self.drs = [],[],[],[] # reset array memory 重置陣列內存
